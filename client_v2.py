...
import os
import sys
from metastore2 import ErrorResponse
import time
"""
...
Client selects blockstores based on RTT instead of deterministic distribution
"""
			...
			try:
				...
				if missing_hashlist:
					...
					for hash in missing_hashlist:
						#print("store block")
						block_number = self.findServer()
						self.blockStoreList[block_number].root.store_block(hash, self.hash_dict[hash])
						self.metaDataStore.root.store_location(hash, block_number)
...
#				hashlist = json.loads(hashlist)
				...
				for hash in hashlist:
					#print("ye")
					#print(hash)
					block_number = self.metaDataStore.root.get_location(hash)
					block = self.blockStoreList[block_number].root.get_block(hash)
					frame.extend(block)
				frame = "".join(map(str, frame))
				fildir = open(fpath, 'w')
				fildir.write(frame)
				fildir.close()
				t2 = time.time()
				print("OK")
				file = open("download_records.txt", "a+")
				file.write(filename + ": " + str(t2-t1)+"\r\n")
			except ErrorResponse:
				print("Not Found")

	def findServer(self):
		"""New def checks shortest time and returns the appropriate server number"""
		# return (int(h,16)) % self.no_of_blockstores
		shortest_time = 100
		cnt = -1
		for x in self.blockStoreList:
			wait = True
			t1 = time.time()
			wait = x.root.ping_store()
			while wait:
				pass
			t2 = time.time()
			if t2-t1 < shortest_time:
				cnt += 1
		return cnt
