...
This version of blockstore also uses the location based loading where it has
a function that helps the client know the RTT of the blockstore and the client
"""
class BlockStore(rpyc.Service):
	...
	"""
	def __init__(self):
		self.Memory = {} # stores the hash : block pair
	...
	def exposed_has_block(self, h):
		return h in self.Memory.keys()

	def exposed_ping_store(self):
		return False
